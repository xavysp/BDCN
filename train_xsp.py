...
from xsploss import xavy_loss, cross_entropy_loss2d,bdcn_loss2d
...
def adjust_learning_rate(optimizer, steps, step_size, gamma=0.1, logger=None):
    ...
    optimizer.defaults['lr'] = optimizer.defaults['lr'] * gamma
    if logger:
        print('%s: %s' % (logger.info('%s: %s' % ('efge', optimizer.defaults['lr']))))
...
def train(model, args, device='gpu'):
    ...
    if logger:
        ...
        logger.info('%s: %s' % ('efge', optimizer.defaults['lr']))
    ...
    for step in range(start_step, args.max_iter + 1):
        ...
        for i in range(args.iter_size):
            ...
            out = model(images)
            # loss = 0
            loss = sum([criterion(preds, labels,l_w)/batch_size for preds, l_w in zip(out, l_weights)])  # BDCN
            # for k in range(10):
...
def parse_args():
    ...
    parser.add_argument('--lr', dest='base_lr', type=float, default=1e-6,
        help='the base learning rate of model') # initial 1e-2
    ...
    parser.add_argument('--complete-pretrain', type=str, default=None,#"datasets/final-model/bdcn_pretrained_on_bsds500.pth",
        help='finetune on the complete_pretrain, default None')  #None to train from the scratch, this is for re train
